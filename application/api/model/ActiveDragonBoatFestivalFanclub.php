...
class ActiveDragonBoatFestivalFanclub extends Base
{
    public static function getList($uid,$active_id,$page,$size)
    {
        ...
        $list=self::where('active_id',$active_id)->order('total_count desc')->page($page, $size)->select();
        $list = json_decode(json_encode($list),TRUE);
        foreach ($list as &$value){
            $star_id=Fanclub::where('id',$value['fanclub_id'])->value('star_id');
            $value['star_name']=Star::where('id',$star_id)->value('name');
            if($fanclub_id && $value['fanclub_id']==$fanclub_id){
                $value['is_mine']=true;
            }else{
                $value['is_mine']=false;
            }
    ...
    public static function getUserList($uid,$page,$size)
    {
        ...
        $dragon_boat_festival_hot_members=FanclubUser::order('dragon_boat_festival_hot desc')->column('user_id');
        $result['myinfo']['dragon_boat_festival_hot'] = FanclubUser::where('user_id', $uid)->value('dragon_boat_festival_hot');
